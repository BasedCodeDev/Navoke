...
import {
  abortableDelay, acquireExport, isModelReady, isModelRunning, manualActionReason,
  modelFailure, modelMetadata, projectApiBase, referenceUrls, verifyRetainedReferences
} from "./studioHelpers";

export const HUNYUAN_GLOBAL_WORKFLOW_ID = "navoke.hunyuan.global.image-to-model";
export const HUNYUAN_GLOBAL_TARGET_URL = "https://hy3d.tencent.ai/studio/creation/prop/geo";
export const STUDIO_SELECTORS = {
  multiView: "text=/^Multi-View$/",
  addViews: "text=/^Add Multi-View \\(Min2, Max8\\)$/",
  viewCard: (n: number) => `[class*="h-[730px]"] > div:nth-child(${n})`,
  faceCount: "text=/^50k$/",
  generate: "text=/^Generate Now$/",
  retopology: 'a[href="/studio/creation/prop/poly"]',
  topology: (value: "Quads" | "Triangles") => `text=/^${value}$/`,
  medium: "text=/^Medium$/",
...
interface ElementState { count: number; text: string; visible: boolean; enabled?: boolean }
interface ImageResult { images: Array<{ src: string }> }
interface LabSession { id: string }
interface LabWait { satisfied: boolean; reason?: string }
...
export function createWorkflows(sdk: WorkflowSdk): WorkflowDefinition[] {
  ...
  const workflow: WorkflowDefinition<ReturnType<typeof inputSchema.parse>, ReturnType<typeof outputSchema.parse>> = {
    manifest: {
      ...
      targetUrl: HUNYUAN_GLOBAL_TARGET_URL, outputKinds: ["model", "json"],
      inputFields: [
    ...
    async run(input, ctx) {
      ...
      const apiBase = projectApiBase(ctx.paths.projectDir);
      for (const file of [input.frontImage, input.backImage]) {
        if (!fs.statSync(file).isFile()) throw new Error(`Reference is not a file: ${file}`);
      }
      const target: ExtensionBrowserTarget = {
        mode: "new", routingToken: crypto.randomUUID(), url: HUNYUAN_GLOBAL_TARGET_URL, openMode: "window", background: true
      };
      let ownedClient: ExtensionClientStatus | undefined;
      let session: LabSession | undefined;
      let pausedMs = 0;
      const artifactIds: string[] = [];
      const options = { signal: ctx.signal, timeoutMs: 30_000 };

      async function request<T>(route: string, body?: unknown, method = "POST", cleanup = false): Promise<T> {
      ...
      async function read<T>(query: unknown): Promise<T> {
        for (;;) {
          ctx.signal.throwIfAborted();
          try { return await browser.extract(target, query, options) as T; }
          catch (error) {
            ctx.signal.throwIfAborted();
            if (browser.findCompatibleClientForTarget(target)) throw error;
      ...
      async function text(): Promise<string> {
        for (;;) {
          const body = String((await read<{ text: string }>({ kind: "text" })).text || "");
          const reason = manualActionReason(body);
          if (!reason) return body;
          await manual(reason);
        }
      }
      const state = (selector: string) => read<ElementState>({ kind: "element-state", selector });
      const images = () => read<ImageResult>({ kind: "images" });
      async function capture(label: string): Promise<void> {
        await saveJson(`lab-${label}.json`, await request(route("/inspect")));
      }
      async function waitFor(check: () => Promise<boolean>, label: string, timeoutMs = 180_000): Promise<void> {
        const start = Date.now();
        const initialPaused = pausedMs;
        while (Date.now() - start - (pausedMs - initialPaused) < timeoutMs) {
          ctx.signal.throwIfAborted();
          if (await check()) return;
          await abortableDelay(2_000, ctx.signal);
        }
        throw new Error(`Timed out: ${label}`);
      }
      const action = (value: unknown) => request(route("/actions"), { action: value });
      async function click(selector: string): Promise<void> {
        // Check human gates before an action. Never blindly retry a submitted generation.
        await text();
        const ready = await request<LabWait>(route("/wait"), { condition: { kind: "element", selector, state: "visible", timeoutMs: 30_000 } });
        if (!ready.satisfied) {
          await text();
      ...
      async function generate(label: string, progress: number): Promise<void> {
        await ctx.step(label, progress);
        await capture(`before-${progress}`);
        await click(STUDIO_SELECTORS.generate);
        await waitFor(async () => {
          const body = await text();
          const failure = modelFailure(body);
          if (failure) throw new Error(`${label} reported: ${failure}`);
          return isModelRunning(body);
        }, `${label} start`, 60_000);
        ...
        await waitFor(async () => {
          ...
          const failure = modelFailure(body);
          if (failure) throw new Error(`${label} reported: ${failure}`);
          return isModelReady(body) && (await state(STUDIO_SELECTORS.download)).visible;
        }, `${label} completion`, 1_800_000);
        await capture(`complete-${progress}`);
      }
      async function uploadViews(): Promise<void> {
        await click(STUDIO_SELECTORS.multiView);
        await click(STUDIO_SELECTORS.addViews);
        for (const [n, file, label] of [[2, input.frontImage, "Front View"], [3, input.backImage, "Back View"]] as const) {
          const selector = STUDIO_SELECTORS.viewCard(n);
          const card = await state(selector);
          if (card.count !== 1 || !card.text.includes(label)) throw new Error(`Unexpected multiview slot layout for ${label}.`);
          await action({ kind: "attach-file", selector: `${selector} input[type=file]`, filePaths: [file] });
          await waitFor(async () => {
            await text();
            const result = await state(selector);
            if (/failed|unsupported/i.test(result.text)) throw new Error(`Reference rejected: ${label}; ${result.text}`);
      ...
      try {
        const system = await request<{ paths: { projectDir: string } }>("/api/system", undefined, "GET");
        if (path.resolve(system.paths.projectDir) !== path.resolve(ctx.paths.projectDir)) throw new Error("Active Navoke runtime belongs to another project.");
        for (;;) {
          try {
        ...
        session = await request<LabSession>("/api/lab/sessions", { mode: "extension", clientId: ownedClient.id, targetUrl: ownedClient.url });
        ...
        await uploadViews();
        const originals = input.topology === "Original" ? referenceUrls((await images()).images) : [];
        if (input.topology === "Original" && originals.length !== 2) throw new Error("Expected two uploaded reference URLs before preserving geometry.");
        await click(STUDIO_SELECTORS.faceCount);
        await generate("Generating 50k prop geometry", 25);
        if (input.topology !== "Original") {
          await click(STUDIO_SELECTORS.retopology);
          await click(STUDIO_SELECTORS.topology(input.topology));
          await click(STUDIO_SELECTORS.medium);
        ...
        await click(STUDIO_SELECTORS.texture);
        await click(STUDIO_SELECTORS.imageToTexture);
        if (input.topology === "Original") {
          await click(STUDIO_SELECTORS.multiView);
          verifyRetainedReferences(originals, (await images()).images);
        ...
        const release = await acquireExport(ctx.signal);
        try {
          await ctx.step("Opening GLB export menu", 90);
          await text();
          await action({ kind: "hover", selector: STUDIO_SELECTORS.exportHover });
          await capture("export-menu");
          const watch = browser.startDownloadWatch();
          await click(STUDIO_SELECTORS.glb);
          ...
          const metadata = modelMetadata(input, fs.readFileSync(download.filename));
          const modelPath = path.join(ctx.artifactDir, "prop.glb");
          fs.copyFileSync(download.filename, modelPath);
