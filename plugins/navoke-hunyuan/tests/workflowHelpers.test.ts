...
describe("Hunyuan workflow helpers", () => {
  ...
  it("registers Hunyuan Global as an extension-routed workflow with a default new tab target", () => {
    ...
    expect(parsed.data).toMatchObject({
      ...
      extensionTab: {
        ...
        url: expect.stringContaining("https://3d.hunyuanglobal.com/#navoke-tab="),
        openMode: "window"
      }
    });
  });
  ...
  it("closes the Navoke-opened Hunyuan Global tab after workflow failure", async () => {
  ...
  it("reports a profile-safe manual action when Hunyuan Global has tabs but no browser controller", async () => {
    ...
    sdk.extension.browser = {
      ...
      ensureRoutedTab: async () => {
        throw new Error(
          "No compatible Navoke browser controller with open-window support is connected. Do not open chrome.exe or paste routed workflow URLs into another Chrome profile."
        );
      }
    };
  ...
  it("reports site-access guidance when Hunyuan Global opens but the routed page does not connect", async () => {
    ...
    sdk.extension.browser = {
      ...
      ensureRoutedTab: async () => {
        throw new Error(
          'Opened a routed Navoke browser window, but no compatible page client connected. openResult={"ok":true,"injection":{"injected":false,"reason":"Cannot access contents of url"}}'
        );
      }
    };
