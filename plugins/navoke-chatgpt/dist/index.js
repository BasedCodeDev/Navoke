...
exports.normalizeChatGptExtensionPromptImageOutputs = normalizeChatGptExtensionPromptImageOutputs;
exports.normalizeChatGptExtensionImagePromptOutputs = normalizeChatGptExtensionImagePromptOutputs;
...
function createWorkflows(sdk) {
    ...
    const chatGptExtensionImageTransformWorkflow = {
        manifest: {
            id: "navoke.chatgpt.extension-image-transform",
                ...
                while (true) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (isOperationCancelled(focusError))
                                throw focusError;
                    ...
                    try {
                        ...
                        if (error instanceof MissingExtensionTabError) {
                            if (phase === "subject" && phaseInput.subjectIndex !== undefined && (promptSubmitted || submittedBaseline !== undefined)) {
                                pausedSubject = {
                                    subjectIndex: phaseInput.subjectIndex,
                                    reason: "ChatGPT tab disconnected after subject submission started. Resume will inspect the current page for an output before resubmitting this subject.",
    ...
    const chatGptExtensionImageSequenceWorkflow = {
        manifest: {
            id: "navoke.chatgpt.extension-image-sequence",
                        ...
                        if (error instanceof MissingExtensionTabError) {
                            if (phase === "subject" && phaseInput.subjectIndex !== undefined && (promptSubmitted || submittedBaseline !== undefined)) {
                                ...
                                pausedPrompt = {
                                    promptIndex: phaseInput.subjectIndex,
                                    ...(pausedInputImagePath ? { inputImage: pausedInputImagePath } : {}),
                                    reason: "ChatGPT tab disconnected after prompt submission started. Resume will inspect the current page for an output before resubmitting this prompt.",
                                    ...(submittedBaseline !== undefined ? { baseline: submittedBaseline } : {})
                                };
                                await persistCheckpointOutput(`ChatGPT tab disconnected after ${phaseLabel} submit; checking for existing output.`);
                                return {
    ...
    const chatGptExtensionPromptImageWorkflow = {
        manifest: {
            id: "navoke.chatgpt.extension-image-prompt",
        ...
        async run(input, ctx) {
            ...
            if (restored.outputMapping && restored.output) {
                ...
                usedOutputNames.add(node_path_1.default.basename(restored.outputMapping.outputPath));
            }
            latestChatGptPage = restored.chatGptPage ?? latestChatGptPage;
            pausedPrompt = restored.pausedPrompt;
            const checkpointOutput = (summary) => ({
            ...
            const registerOutputArtifact = async (output, taskId) => {
                ...
                const mimeType = output.mimeType ?? "image/png";
                const extension = extensionForMimeType(mimeType);
                const outputPath = node_path_1.default.join(artifactDir, outputFileNameForPromptImage(input.prompt, extension, usedOutputNames));
                node_fs_1.default.writeFileSync(outputPath, Buffer.from(output.base64, "base64"));
                const artifact = await ctx.addArtifact({
                    kind: "image",
                    name: node_path_1.default.basename(outputPath),
            ...
            const runPromptTask = async (phaseLabel, taskInput) => {
                while (true) {
                    const { target, client } = await waitForTargetAtCheckpoint(phaseLabel);
            ...
            while (!outputKey) {
                if (pausedPrompt) {
                    ...
                    if (pausedPrompt)
                        continue;
                    await ctx.event("chatgpt.capture_existing.missed", "Could not capture an existing ChatGPT output for prompt image; resubmitting the prompt.");
                }
                await runPromptTask("prompt image", { subjectMode: "submit-and-capture" });
                await outputProcessing;
                if (outputProcessingError)
                    throw outputProcessingError;
                if (pausedPrompt && !outputKey)
                    continue;
                if (!outputKey)
            ...
            const manifestPath = node_path_1.default.join(artifactDir, "chatgpt-extension-prompt-image-manifest.json");
            writeJson(manifestPath, {
                prompt: input.prompt,
                extensionTab: redactTargetForManifest(input.extensionTab),
                chatGptPage: latestChatGptPage ?? null,
    ...
    const chatGptExtensionImagePromptWorkflow = {
        manifest: {
            id: "navoke.chatgpt.extension-image-prompt-transform",
        ...
        async run(input, ctx) {
            ...
            const registerOutputArtifact = async (output, taskId) => {
                ...
                const mimeType = output.mimeType ?? "image/png";
                const extension = extensionForMimeType(mimeType);
                const outputPath = node_path_1.default.join(artifactDir, outputFileNameForImagePrompt(input.image, input.prompt, extension, usedOutputNames));
                node_fs_1.default.writeFileSync(outputPath, Buffer.from(output.base64, "base64"));
                const artifact = await ctx.addArtifact({
                    kind: "image",
                    name: node_path_1.default.basename(outputPath),
            ...
            const runImagePromptTask = async (phaseLabel, taskInput) => {
                while (true) {
                    const { target, client } = await waitForTargetAtCheckpoint(phaseLabel);
                    await ctx.step(`Queued ChatGPT ${phaseLabel} task`, 20, {
                        phase: "subject",
                    ...
                    try {
                        ...
                        if (pausedMetadata) {
                            ...
                            await persistCheckpointOutput(`Paused during ChatGPT ${phaseLabel}.`);
                            await ctx.waitForManualAction(`Paused during ChatGPT ${phaseLabel}. Resume will try to capture the current page output before resubmitting this image prompt.`, manualActionData(phaseLabel));
                            return result;
                        }
                        await persistCheckpointOutput(`Completed ChatGPT ${phaseLabel} task.`);
                        return result;
                        ...
                        if (error instanceof ImmediateChatGptPauseError) {
                            pausedPrompt = {
                                reason: "Paused immediately. Resume will inspect the current ChatGPT page for an output before resubmitting this image prompt.",
                                ...(taskInput.subjectBaseline !== undefined ? { baseline: taskInput.subjectBaseline } : {})
                            };
                            await persistCheckpointOutput(`Paused during ChatGPT ${phaseLabel}.`);
                            await ctx.waitForManualAction(`Paused during ChatGPT ${phaseLabel}. Refresh the ChatGPT page if needed, then resume. Resume will inspect the current page for an output before resubmitting this image prompt.`, manualActionData(phaseLabel));
                            return {
                                outputs: [],
                                metadata: {
                                    paused: true,
                        ...
                        if (error instanceof MissingExtensionTabError) {
                            if (promptSubmitted || submittedBaseline !== undefined) {
                                pausedPrompt = {
                                    reason: "ChatGPT tab disconnected after image prompt submission started. Resume will inspect the current page for an output before resubmitting this image prompt.",
                                    ...(submittedBaseline !== undefined ? { baseline: submittedBaseline } : {})
                                };
                                await persistCheckpointOutput(`ChatGPT tab disconnected after ${phaseLabel} submit; checking for existing output.`);
                                return {
            ...
            while (!outputKey) {
                if (pausedPrompt) {
                    ...
                    if (pausedPrompt)
                        continue;
                    await ctx.event("chatgpt.capture_existing.missed", "Could not capture an existing ChatGPT output for image prompt; resubmitting the image prompt.");
                }
                ...
                if (pausedPrompt && !outputKey)
                    continue;
                if (!outputKey)
                    throw new Error("ChatGPT extension did not return an output image for the image prompt.");
            }
            ...
            const manifestPath = node_path_1.default.join(artifactDir, "chatgpt-extension-image-prompt-manifest.json");
            writeJson(manifestPath, {
                image: input.image,
                prompt: input.prompt,
                extensionTab: redactTargetForManifest(input.extensionTab),
    ...
    function buildImagePromptPresentation(input, outputMapping, manifestArtifactId) {
        return {
    ...
    function canResumeFailedChatGptRun(run) {
        if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-transform")
            return false;
    ...
    function canResumeFailedChatGptSequenceRun(run) {
        if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-sequence")
            return false;
    ...
    function canResumeFailedChatGptPromptImageRun(run) {
        if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-prompt")
            return false;
    ...
    function readStoredPromptImageOutput(value) {
        const parsed = promptImageOutputSchema.safeParse(value);
        return parsed.success ? parsed.data : null;
    }
    function restorePromptImageCheckpointState(previousOutput, input, artifactDir) {
    ...
    function restorePromptImageOutputMapping(mapping, input, artifactDir) {
        if (!mapping)
            return null;
        if (mapping.prompt !== input.prompt)
            return null;
        if (!mapping.outputPath || !node_fs_1.default.existsSync(mapping.outputPath))
            return null;
        const outputPath = node_path_1.default.resolve(mapping.outputPath);
    ...
    function resolvePausedPromptImageForResume(storedPausedPrompt) {
        return storedPausedPrompt ? { pausedPrompt: storedPausedPrompt } : {};
    }
    function canResumeFailedChatGptImagePromptRun(run) {
        if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-prompt-transform")
            return false;
    ...
    function readStoredImagePromptOutput(value) {
        const parsed = imagePromptOutputSchema.safeParse(value);
        return parsed.success ? parsed.data : null;
    }
    function restoreImagePromptCheckpointState(previousOutput, input, artifactDir) {
    ...
    function restoreImagePromptOutputMapping(mapping, input, artifactDir) {
        if (!mapping)
            return null;
        if (mapping.image !== input.image)
            return null;
        if (mapping.prompt !== input.prompt)
            return null;
        if (!mapping.outputPath || !node_fs_1.default.existsSync(mapping.outputPath))
        ...
        return {
            ...
            output: {
                subjectIndex: 0,
                subjectName: node_path_1.default.basename(mapping.image),
                name: node_path_1.default.basename(outputPath),
                mimeType,
                base64: node_fs_1.default.readFileSync(outputPath).toString("base64"),
                metadata: {
    ...
    function chatGptControllerManualMessage(error) {
        ...
        if (/controller/i.test(message)) {
            return "Reload or install the Navoke browser extension in the intended browser profile, open any page or the extension popup so the controller connects, then resume this run.";
        }
        return "The Navoke browser controller could not open or connect to the tracked ChatGPT page. Reload the extension in the intended browser profile, then resume this run.";
    }
    ...
    function isRecoverableExtensionCommandError(error) {
        ...
        return (/Timed out waiting for browser extension command/i.test(error.message) ||
            /No compatible Navoke browser extension tab is connected/i.test(error.message) ||
...
async function focusChatGptTarget(browser, target, task, signal, emit, force = false) {
    const now = Date.now();
    if (!force && task.lastFocusedAt && now - task.lastFocusedAt < CHATGPT_FOCUS_THROTTLE_MS)
        return;
...
async function captureChatGptSubmitBaseline(browser, target, selectors, task, signal, emit) {
...
async function submitChatGptPrompt(browser, sleep, target, selectors, prompt, options, task, emit, beforeSubmit) {
    if (prompt.trim()) {
...
async function waitForChatGptOutput(browser, sleep, target, selectors, baseline, options, task) {
    ...
    throw new Error(`Timed out waiting for a new ChatGPT output image. Capture diagnostics: ${JSON.stringify(summarizeCaptureDiagnostics(lastDiagnostics))}`);
}
async function extractChatGptImageBaseline(browser, target, selectors, signal) {
    const result = normalizeRecord(await browser.extract(target, { kind: "images", selector: selectors.outputImage, minWidth: 128, minHeight: 128 }, { signal, timeoutMs: 30_000 }));
    const images = Array.isArray(result.images) ? result.images.map(normalizeRecord) : [];
...
function normalizeChatGptImageBaseline(value) {
...
function selectChatGptOutputCandidate(images, baseline, uploadedSubjectKey, task) {
    ...
    const rawCandidates = images.map((image) => normalizeChatGptImageCandidate(image, baselineFingerprintSet, baselineStableSourceIdSet, uploadedSubjectKey));
    const candidates = dedupeChatGptImageCandidates(rawCandidates);
    const diagnostics = buildCaptureDiagnostics(candidates, baselineFingerprintSet.size, baselineStableSourceIdSet.size, rawCandidates.length);
    ...
    if (generated.length === 1)
        return { candidate: generated[0], reason: "generated-image-label", diagnostics };
    if (generated.length > 1)
        throw multipleChatGptOutputCandidatesError(task, generated, diagnostics, "generated image candidates");
    const assistant = eligible.filter((candidate) => candidate.assistantLike);
    if (assistant.length === 1)
        return { candidate: assistant[0], reason: "assistant-context", diagnostics };
    if (assistant.length > 1)
        throw multipleChatGptOutputCandidatesError(task, assistant, diagnostics, "assistant image candidates");
    if (eligible.length === 1)
        return { candidate: eligible[0], reason: "single-unambiguous-fallback", diagnostics };
    if (eligible.length > 1)
        throw multipleChatGptOutputCandidatesError(task, eligible, diagnostics, "fallback image candidates");
    return { reason: "no-eligible-candidate", diagnostics };
}
function normalizeChatGptImageCandidate(image, baselineFingerprintSet, baselineStableSourceIdSet, uploadedSubjectKey) {
    ...
    const messageRole = firstNonEmptyString(stringValue(image.messageRole), stringValue(ancestorAttributes["data-message-author-role"])) ?? "";
    ...
    const assistantLike = /^assistant$/i.test(messageRole) ||
        generated ||
        (/chatgpt said/i.test(ancestorText) && !/^user$/i.test(messageRole) && !/\byou said\b/i.test(ancestorText));
    const rejectionReasons = [];
    if (!base64)
        rejectionReasons.push("missing-base64");
    if (fingerprint && baselineFingerprintSet.has(fingerprint))
        rejectionReasons.push("baseline-fingerprint");
    if (stableSourceId && baselineStableSourceIdSet.has(stableSourceId))
        rejectionReasons.push("baseline-stable-source-id");
    if (uploadedSubjectKey && base64 && base64IdentityKey(base64) === uploadedSubjectKey)
        rejectionReasons.push("uploaded-byte-identical");
    if (uploadLike)
        rejectionReasons.push("upload-or-user-context");
    return {
...
function isUploadLikeImageCandidate(input) {
    if (input.insideForm || input.insideEditable)
        return true;
    if (/^user$/i.test(input.messageRole))
        return true;
    const label = `${input.ancestorRole} ${input.ancestorAriaLabel}`.toLowerCase();
    if (/\b(?:composer|prompt|attachment|uploaded|upload|input)\b/.test(label))
        return true;
    const text = input.ancestorText.toLowerCase();
    return /\byou said\b/.test(text) && !/\bchatgpt said\b/.test(text);
}
function dedupeChatGptImageCandidates(candidates) {
...
function chatGptImageCandidateIdentity(candidate) {
    return (firstNonEmptyString(candidate.stableSourceId ? `stable:${candidate.stableSourceId}` : "", candidate.fingerprint ? `fingerprint:${candidate.fingerprint}` : "", candidate.src ? `src:${candidate.src}` : "", candidate.base64 ? `base64:${base64IdentityKey(candidate.base64)}` : "") ?? `dom:${candidate.domIndex}`);
}
function mergeChatGptImageCandidates(a, b) {
    const chosen = chatGptImageCandidateScore(b) > chatGptImageCandidateScore(a) ? b : a;
    const rejectionReasons = uniqueStrings([...a.rejectionReasons, ...b.rejectionReasons]).filter((reason) => reason !== "missing-base64" || !chosen.base64);
    return {
        ...chosen,
...
function chatGptImageCandidateScore(candidate) {
    return ((candidate.rejectionReasons.length === 0 ? 32 : 0) +
...
function buildCaptureDiagnostics(candidates, baselineFingerprintCount, baselineStableSourceIdCount, totalImages = candidates.length) {
    const eligible = candidates.filter((candidate) => candidate.rejectionReasons.length === 0);
    return {
        totalImages,
...
function multipleChatGptOutputCandidatesError(task, candidates, diagnostics, candidateKind) {
    const label = Number.isInteger(task.subjectIndex) ? `prompt ${task.subjectIndex + 1}` : "the current prompt";
    return new Error(`ChatGPT returned ${candidates.length} distinct output image candidates for ${label}. ` +
...
function summarizeCaptureDiagnostics(value) {
    if (!value)
        return null;
    return {
...
function baselineStableSourceIds(value) {
    if (!value || typeof value !== "object")
        return [];
    const record = value;
    return Array.isArray(record.stableSourceIds)
        ? record.stableSourceIds.filter((item) => typeof item === "string")
        : [];
}
...
function buildChatGptPage(target, metadata, targetClient) {
    ...
    const routingToken = firstNonEmptyString(target.mode === "new" ? target.routingToken : undefined, targetClient?.routingToken, pageMetadata.routingToken, targetRecord.routingToken);
...
function safePromptStem(prompt) {
...
function outputFileNameForPromptImage(prompt, extension, usedNames) {
    const baseName = `${safePromptStem(prompt)}-chatgpt`;
    return uniqueOutputFileName(baseName, extension, usedNames);
}
function outputFileNameForImagePrompt(sourceImage, prompt, extension, usedNames) {
    const baseName = `${safeStem(sourceImage)}-${safePromptStem(prompt)}-chatgpt`;
    return uniqueOutputFileName(baseName, extension, usedNames);
}
function uniqueOutputFileName(baseName, extension, usedNames) {
    let candidate = `${baseName}.${extension}`;
    let duplicate = 2;
    while (usedNames.has(candidate)) {
        candidate = `${baseName}-${duplicate}.${extension}`;
...
function sequenceSetupPrompt(input) {
    if (!input.masterPrompt || !input.masterPromptSuffix)
...
function normalizeChatGptExtensionPromptImageOutputs(outputs) {
    const outputsByIdentity = new Map();
    for (const output of outputs) {
        if (output.subjectIndex !== 0) {
            throw new Error(`ChatGPT extension returned output for unexpected prompt index ${output.subjectIndex}.`);
        }
    ...
    if (distinctOutputs.length > 1) {
        throw new Error(`ChatGPT extension returned ${distinctOutputs.length} distinct output images for the prompt. ` +
            "This workflow expects exactly one result.");
    }
    return distinctOutputs[0];
}
function normalizeChatGptExtensionImagePromptOutputs(outputs) {
...
function imageFileBase64IdentityKey(filePath) {
    if (!filePath)
        return null;
    try {
        return base64IdentityKey(node_fs_1.default.readFileSync(filePath).toString("base64"));
    }
    catch {
        return null;
    }
}
function base64IdentityKey(base64) {
    return (0, node_crypto_1.createHash)("sha256").update(base64).digest("hex");
}
