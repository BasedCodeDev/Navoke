...
describe("ChatGPT plugin browser-extension workflows", () => {
  ...
  it("uses generic extension tab routing instead of site-specific extension capabilities", () => {
    const workflow = workflows.find((candidate) => candidate.manifest.id === "navoke.chatgpt.extension-image-transform");
  ...
  it("defaults the extension tab target to a routed new window", () => {
    ...
    expect(extensionTab.url).toContain("navoke-tab=");
  });
  ...
  it("defaults the prompt-image workflow to a routed new window", () => {
    const workflow = workflows.find((candidate) => candidate.manifest.id === "navoke.chatgpt.extension-image-prompt")!;
    const parsed = workflow.inputSchema.safeParse({
  ...
  it("defaults the image-prompt workflow to a routed new window and one source image", () => {
    const workflow = workflows.find((candidate) => candidate.manifest.id === "navoke.chatgpt.extension-image-prompt-transform")!;
    const parsed = workflow.inputSchema.safeParse({
  ...
  it("resumes a routed window checkpoint by captured extension client id", async () => {
    ...
    const { run } = runTransformWithFakeBrowser({
      ...
      extensionTab: {
        ...
        url: "https://chatgpt.com/#navoke-tab=route-token"
      },
      previousOutput: {
        artifactIds: [],
  ...
  it("recovers automatically when a routed ChatGPT tab closes and the controller can reopen it", async () => {
    ...
    const { run } = runTransformWithFakeBrowser({
      ...
      extensionTab: {
        mode: "new",
        routingToken: "route-token",
        url: "https://chatgpt.example/#navoke-tab=route-token"
      },
      previousOutput: {
        artifactIds: [],
        summary: "Failed after setup.",
  ...
  it("runs fresh routed tabs through the captured extension client id after connection", async () => {
    const { commandTargets, updates, run } = runTransformWithFakeBrowser({
      ...
      extensionTab: {
        ...
        url: "https://chatgpt.com/#navoke-tab=fresh-route-token"
      }
    });
  ...
  it("submits one source image with one prompt and captures one generated image", async () => {
    ...
    const { actions, artifacts, run } = runPromptImageWithFakeBrowser({
      workflowId: "navoke.chatgpt.extension-image-prompt-transform",
      image: "C:\\tmp\\source.png",
      prompt: "Remove the background.",
      imageExtractResult: {
        images: [
...
function runTransformWithFakeBrowser(input: {
  ...
  sdk.extension.browser = {
    ...
    findCompatibleClientForTarget:
      ...
      ((target) => ({
        id: target.mode === "existing" ? target.clientId : "client-1",
        url: target.mode === "new" ? (target.url ?? "https://chatgpt.example/#navoke-tab=token") : "https://chatgpt.example/",
    ...
    ensureRoutedTab: async (target) => {
      ...
      const client =
        (input.findCompatibleClientForTarget ??
          ((candidate) => ({
            ...
            url: candidate.mode === "new" ? (candidate.url ?? "https://chatgpt.example/#navoke-tab=token") : "https://chatgpt.example/",
            title: "ChatGPT test tab",
            status: "connected",
            protocolVersion: 1,
            extensionVersion: "0.1.0",
      ...
      if (!client) throw new Error("No compatible Navoke browser controller is connected.");
      return client;
    },
  ...
  const workflow = createWorkflows(sdk).find((candidate) => candidate.manifest.id === "navoke.chatgpt.extension-image-transform")!;
...
function runPromptImageWithFakeBrowser(input: {
  ...
  const workflowId = input.workflowId ?? "navoke.chatgpt.extension-image-prompt";
...
function runSequenceWithFakeBrowser(input: {
  ...
  const workflow = createWorkflows(sdk).find((candidate) => candidate.manifest.id === "navoke.chatgpt.extension-image-sequence")!;
