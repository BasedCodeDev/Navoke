...
function buildSequencePresentation(input: ChatGptSequenceWorkflowInput, outputMappings: SequenceOutputMapping[], manifestArtifactId: string): Record<string, unknown> {
  return {
...
function buildPromptImagePresentation(
  input: ChatGptPromptImageWorkflowInput,
  outputMapping: PromptImageOutputMapping | undefined,
...
function canResumeFailedChatGptRun(run: RunRecord): boolean {
  if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-transform") return false;
...
function canResumeFailedChatGptSequenceRun(run: RunRecord): boolean {
  if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-sequence") return false;
...
function canResumeFailedChatGptPromptImageRun(run: RunRecord): boolean {
  if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-prompt") return false;
...
function canResumeFailedChatGptImagePromptRun(run: RunRecord): boolean {
  if (run.status !== "failed" || run.workflowId !== "navoke.chatgpt.extension-image-prompt-transform") return false;
