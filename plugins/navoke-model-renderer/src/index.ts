...
export const MODEL_RENDER_IMAGE_WORKFLOW_ID = "navoke.model-renderer.render-image";
export const MODEL_GEOMETRY_BOUNDS_WORKFLOW_ID = "navoke.model-renderer.geometry-bounds";
...
async function renderModelImage(
  ...
  const result = await withRendererPage(sdk, ctx, assets, request.width, request.height, async (page) => {
    ...
    return (await page.evaluate((renderRequest: RenderRequest) => {
      return (window as any).navokeModelRenderer.render(renderRequest) as Promise<RenderResult>;
...
async function readModelBounds(sdk: WorkflowSdk, ctx: WorkflowContext, assets: ModelAssetDiscovery): Promise<ModelBounds> {
  ...
  const bounds = await withRendererPage(sdk, ctx, assets, 256, 256, async (page) => {
    ...
    return (await page.evaluate((model: BrowserModelDescriptor) => {
      return (window as any).navokeModelRenderer.bounds(model) as Promise<ModelBounds>;
...
async function withRendererPage<T>(
  ...
  try {
    ...
    await page.waitForFunction(() => Boolean((window as any).navokeModelRenderer), undefined, { timeout: 30_000 });
...
function resolveAppPackageRoot(sdk: WorkflowSdk, packageName: string): string {
  ...
  try {
    ...
    throw new Error(`Unable to resolve bundled package "${packageName}". Restart Navoke after updating the app. ${message}`);
  }
}
