---
name: navoke-model-renderer
description: Use when working with the Navoke model renderer plugin, including rendering OBJ/FBX/Hunyuan ZIP model inputs to PNG images, extracting geometry bounds, preparing CLI inputs for `navoke.model-renderer.render-image` or `navoke.model-renderer.geometry-bounds`, and interpreting model render artifacts.
---

# Navoke Model Renderer

Use the Navoke CLI skill for runtime discovery, plugin installation, watching runs, and artifact lookup. This skill only covers the model renderer workflow inputs and artifact expectations.

## Workflows

- `navoke.model-renderer.render-image`: render a model input to a PNG image artifact.
- `navoke.model-renderer.geometry-bounds`: return original model-coordinate geometry bounds as JSON.
