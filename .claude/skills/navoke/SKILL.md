...
All output is JSON. Parse stdout as JSON for normal commands and newline-delimited JSON for `navoke watch` or `navoke run --wait`.

Mutating or control commands must not rely on the default port fallback. For `run`, `plugin-install`, `pause`, `resume`, `cancel`, and `delete`, pass `--project <project-dir>`, pass `--api-url <url>`, or use `NAVOKE_API_URL`.
...
```powershell
navoke --project <project-dir> run <workflowId> --input input.json --agent claude --wait
```
...
When a run reaches `waiting_manual`, report the current step to the user and wait for them to complete the browser action. After they confirm, call `navoke --project <project-dir> resume <runId>` and continue watching.
