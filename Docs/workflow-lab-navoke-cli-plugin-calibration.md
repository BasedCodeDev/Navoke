# Workflow Lab And Navoke CLI Plugin Calibration Loop

## Purpose

Use this loop when building or hardening a browser workflow plugin against a real target site. Workflow Lab is the microscope for learning page state, selectors, uploads, waits, and trace evidence. The Navoke CLI is the repeatable run harness that proves the installed plugin works in the real app runtime and produces artifacts.

The important pattern is not "try a selector and hope". It is:

1. Run the real workflow through Navoke CLI.
...
Navoke CLI owns proof:

- Confirm the app runtime and project with `navoke --project <project-dir> status`.
- Inspect workflow manifests with `navoke --project <project-dir> workflow <workflowId>`.
...
```powershell
navoke.cmd --project <project-dir> status
navoke.cmd --project <project-dir> workflows
navoke.cmd --project <project-dir> workflow <workflowId>
```
...
For `navoke.chatgpt.extension-image-sequence`, remember that CLI input does not inherit renderer prefilled values. If you provide a non-empty `masterPrompt` and want the standard image-only setup guardrail, include `masterPromptSuffix` explicitly:
...
For one-off ChatGPT image generation, use `navoke.chatgpt.extension-image-prompt` with a single `prompt` string, or `navoke.chatgpt.extension-image-prompt-transform` with a single image path in `image` plus `prompt`:
...
```powershell
navoke.cmd --project <project-dir> run <workflowId> --input <input.json> --agent codex --name "<short calibration name>"
navoke.cmd --project <project-dir> watch <runId>
```
...
```powershell
navoke.cmd --project <project-dir> get <runId>
```
